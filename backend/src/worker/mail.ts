...
export class MailWorker extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    const stream: Stream = await this.streamModel
      .findOne({
        streamId,
      })
      .exec();

    const streamer: Account = await this.accountModel
      .findOne({
        address: stream.streamer,
      })
      .populate(['followers', 'channel'])
      .exec();
    ...
    const subject: string = `${streamer.name} from Hurbe.`;
    ...
    const message: string = started
      ? `<p><b>${streamer.channel.name}</b> is starting a live stream now.</p>
            ...
            <br /> <br />
            <a href="https://hurbe.xyz/streams/${streamId}">
