...
const uploadVideo = async () => {
    ...
    const streamResponse = await MediaAPI.createStream(stream.value.name);
    ...
    Storage.upload(stream.value.thumbnail, streamId, async (thumbnailUrl: string) => {
        const upload = await HurbeAPI.createStream(
            streamId,
            walletStore.address!,
            stream.value.name!,
            ...
            <div class="detail_form" v-show="activeTab == 1">
                ...
                <div class="input">
                    <p class="label">Thumbnail <span>*</span>
                        <RouterLink to="/ai">Generate with HurbeAI</RouterLink>
                    </p>
