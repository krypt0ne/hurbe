...
const getVideo = async () => {
    loading.value = true;
    const result = await HurbeAPI.getVideo(route.params.id as any);
    video.value = result;
    if (result) {
        init();
...
const init = async () => {
    ...
    videoUrl.value = await MediaAPI.getVideoUrl(video.value?.thetaId!);
...
const share = () => {
    try {
        navigator.share({
            ...
            url: `https://hurbe.xyz/videos/${video.value?.videoId!}`
        });
    } catch (error) {
        notify.push({
