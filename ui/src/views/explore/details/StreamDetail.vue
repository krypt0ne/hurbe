...
import MediaAPI from '@/scripts/media-api';
import Contract, { hurbeTokenId, hurbeId } from '@/scripts/contract';
...
const getStream = async () => {
    loading.value = true;
    const result = await HurbeAPI.getStream(route.params.id as any);
    stream.value = result;
    if (result) {
        init();
...
const sendComment = async () => {
    ...
    if (tip.value.amount && tip.value.amount > 0) {
        const allowance = await getAllowance(
            hurbeTokenId, walletStore.address, hurbeId
        );
...
const startStream = async () => {
    ...
    starting.value = true;
    let createdStream = await MediaAPI.startStream(stream.value?.thetaId!);

    if (!createdStream) {
        const existingStream = await HurbeAPI.getStream(stream.value?.streamId!);
    ...
    const updatedStream = await HurbeAPI.startStream(
        stream.value?.streamId!,
    ...
    if (stream.value?.streamType == StreamType.External) {
        streamConfigs.value = {
            ...
            data: {
                ...
                stream_reaction: `https://reactions.hurbe.xyz?id=${stream.value?.streamId!}`
            }
        };
...
const init = async () => {
    ...
    videoUrl.value = await MediaAPI.getStreamUrl(stream.value?.thetaId!);
...
const stopStream = async () => {
    ...
    await HurbeAPI.endStream(stream.value?.streamId!);
...
const share = () => {
    try {
        navigator.share({
            ...
            url: `https://hurbe.xyz/streams/${stream.value?.streamId!}`
        });
    } catch (error) {
        notify.push({
...
<template>
    ...
    <div class="detail_container" v-if="stream" v-show="!loading">
        <div class="detail_container_1">
            ...
            <div class="creator">
                <RouterLink :to="`/channels/${(stream.streamer as Account).address}`">
                    ...
                    <div class="comment" v-for="chat, index in chats" :key="index">
                        ...
                        <div class="comment_tipped" v-if="chat.tip.amount > 0">
                            ...
                            <div class="comment_tip_amount">
                                ...
                                <p><span>{{ chat.tip.amount }}</span> HUB</p>
                            </div>
                        </div>
                    </div>
                </div>
