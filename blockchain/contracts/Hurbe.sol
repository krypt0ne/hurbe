...
import {IHurbe} from "./interfaces/IHurbe.sol";
...
contract Hurbe is IHurbe, AccessControl, Pausable {
    ...
    IERC20 private _hurbeToken;

    constructor(address hurbeToken) {
        // Create providers.
    ...
    function claimHurbe(uint256 amount) public override whenNotPaused {
        address streamer = _msgSender();

        Data.Streamer storage streamerData = _streamers[streamer];
        require(streamerData.totalUnClaimedHurbe >= amount, "Insufficient");

        _hurbeToken.transfer(streamer, amount);

        streamerData.totalUnClaimedHurbe -= amount;
        streamerData.totalClaimedHurbe += amount;

        emit ClaimedHurbe(streamer, amount);
    }
    ...
    function claimAll() external override {
        ...
        claimHurbe(streamerData.totalUnClaimedHurbe);
    }
    ...
    function tipStream(
        ...
        _hurbeToken.transferFrom(viewer, address(this), amount);
